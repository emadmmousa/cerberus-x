...
export function FirebreakPanel({ disabled = false }: { disabled?: boolean }) {
  const [status, setStatus] = useState<FirebreakStatus | null>(null);
  ...
  function reloadScaffolds() {
    return Promise.all([
      ...
      setStatus(st as FirebreakStatus);
  ...
  return (
    <div className="panel arsenal" aria-label="Firebreak AI">
      <div className="arsenal__head">
        <div>
          <div className="arsenal__title">Firebreak model</div>
