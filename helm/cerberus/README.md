# Cerberus-X Helm chart (`helm/cerberus`)
...
./helm/deploy.sh
# or:
helm upgrade --install cerberus ./helm/cerberus -n cerberus-x --create-namespace
```
...
| Key | Default | Purpose |
|-----|---------|---------|
| `multiScaffold` | `true` | `CERBERUS_MULTI_SCAFFOLD` |
| `costRoute` | `false` | Prefer cheaper scaffolds first |
| `llmModel` | `cerberus-firebreak` | Primary Ollama / OpenAI-compat model |
...
| `controlPlaneUrl` | `""` | `CERBERUS_CONTROL_PLANE_URL` |
...
Auth0: create secret `cerberus-auth0` with keys `domain`, `clientId`, `clientSecret`, `secret`, then set `firebreak.auth0.enabled=true`.
