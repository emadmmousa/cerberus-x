...
Cerberus-X orchestrates scanners and Metasploit through Celery workers. You launch missions from **Mission Control** (or APIs / MCP). Optional local **Ollama** (`cerberus-firebreak` model) plans dual-posture next steps (aggressive + defensive). Results stream to the UI via Socket.IO + polling.
...
```bash
docker compose up -d
docker compose run --rm ollama-pull   # builds cerberus-firebreak persona
```
...
| Env | Default | Meaning |
|-----|---------|---------|
| `CERBERUS_LLM_BASE_MODEL` | `qwen2.5:7b` | Open base for own Firebreak model |
| `CERBERUS_LLM_MODEL` | `cerberus-firebreak` | Built from `docker/ollama/Modelfile` |
| `CERBERUS_MULTI_SCAFFOLD` | `true` | Query primary + fallback (+ optional EXTRA) |
| `CERBERUS_SCAFFOLD_COST_ROUTE` | `false` | Prefer cheaper scaffolds first |
| `CERBERUS_LLM_THINK` | `false` | Disable thinking traces for clean JSON plans |
...
| `CERBERUS_EDITION` | `community` | `pro` unlocks marketplace register + hosting hooks |
...
When Redis is available, Mission Control uses **server-side Flask sessions** stored under the `cerberus:sess:` prefix (not a signed cookie payload). Each authenticated user gets an isolated session that works across orchestrator replicas.

| Env | Default | Meaning |
|-----|---------|---------|
| `SECRET_KEY` | `change-me` in `.env.example` | Signs session cookies; must not stay at the insecure default `cerberus-x-secret` in production |
| `CERBERUS_SESSION_SECURE` | `false` | Set `true` when serving over HTTPS so session cookies get the `Secure` flag |
...
| **Auto-Train** | Daily merge + eval pipeline; QLoRA only when `CERBERUS_TRAIN_GPU=true` | Crontab hour from `CERBERUS_AUTO_TRAIN_HOUR` (default 3 UTC) |
...
| `CERBERUS_AUTO_TRAIN` | `false` | Defer path for daily ML pipeline |
| `CERBERUS_AUTO_TRAIN_HOUR` | `3` | Hour (UTC) for daily pipeline |
| `CERBERUS_LEARNING_TICK` | `false` | Defer path for per-minute harvest |
| `CERBERUS_TRAIN_GPU` | `false` | Allow real QLoRA in daily pipeline (otherwise dry-run only) |
...
| No scaffolds | Set `CERBERUS_LLM_BASE_URL` (Compose default `http://ollama:11434/v1`) |
...
| Marketplace register 403 | Set `CERBERUS_EDITION=pro` |
